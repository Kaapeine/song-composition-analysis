...
echo ""
echo "==> Step 6: torchcodec (required by torchaudio on newer torch)"
pip install torchcodec

echo ""
echo "==> Step 7: patch allin1/models/dinat.py"
# allin1 was written for the old NATTEN API (natten1dqkrpb / natten1dav).
# Three problems with NATTEN 0.21+ on Apple Silicon (CPU):
...
patches = [
    # 1. Replace NATTEN import with F import (pure functions are inserted in patch 2)
    (
        "from natten.functional import natten1dav, natten1dqkrpb, natten2dav, natten2dqkrpb",
        "import torch.nn.functional as F",
    ),
    # 2. Insert pure-PyTorch attention functions after the wildcard utils import
    (
        "from .utils import *",
        "from .utils import *" + PURE_FUNCTIONS,
    ),
    # 3. Update class-level callable annotations
    (
        "  nattendqkrpb: Callable\n  nattendav: Callable",
        "  nattn_fn: Callable  # _pure_na1d or _pure_na2d",
    ),
    # 4. Replace old QK+RPB/AV forward with pure-PyTorch forward
    (
        OLD_FORWARD,
        NEW_FORWARD,
    ),
    # 5. Update 1D attention function assignment
    (
        "    self.nattendqkrpb = natten1dqkrpb\n    self.nattendav = natten1dav",
        "    self.nattn_fn = _pure_na1d",
    ),
    # 6. Update 2D attention function assignment
    (
