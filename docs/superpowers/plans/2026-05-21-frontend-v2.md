# Music Analyzer Frontend Implementation Plan (v2 — data-first)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a Vite + React + TypeScript SPA served by FastAPI that lets users upload audio, poll analysis progress, and explore a full musical breakdown — getting pages with real data on screen as early as possible, with interactive polish (audio playback, crosshair hover, transposition knob) added last.

**Architecture:** `frontend/` lives alongside `app/` in the monorepo. In dev, Vite on :3000 proxies all API paths to FastAPI on :8000. In production, `npm run build` outputs `frontend/dist/` which FastAPI mounts via `StaticFiles`. Primitive components and data hooks are built before the pages so pages use real components from day one — only audio playback (`AudioProvider`) and shared hover state (`CrosshairProvider`) are deferred to the end because they require non-trivial JS wiring. TanStack Query handles polling and result caching.
...
|---|---|---|
| Backend | 1–3 | APIs ready |
| Foundation | 4–9 | Scaffold + all components built, app shell renders |
| Data hooks | 10 | TanStack Query polling wired |
| Upload flow | 11–12 | Full upload → redirect working |
| Results data | 13–17 | All result cards showing data (static, no audio) |
| Basic assembly | 18 | ResultsPage complete, all data visible |
| Audio polish | 19 | Playback, Transport, playhead on waveform |
| Crosshair + prod | 20 | Hover sync + FastAPI StaticFiles mount |
...
Replace the existing `from app.api import ...` / `app.include_router(...)` lines with:
...
Open `http://localhost:3000` — should show the default Vite + React page.
...
- [ ] **Step 1: Copy theme.css verbatim from design handoff**
...
- [ ] **Step 3: Wire globals into `frontend/src/main.tsx`**
...
In the browser at `http://localhost:3000`, open DevTools → Console:

```js
getComputedStyle(document.documentElement).getPropertyValue('--paper')
```

Expected: `#f1e7d0` (the cream paper colour from theme.css).
...
- [ ] **Step 4: Write `frontend/src/api/client.test.ts`**
...
### Task 9: App shell — routing, Layout, AppHeader
...
`UploadPage` and `ResultsPage` don't exist yet — TypeScript will error. Create placeholder stubs so routing compiles:

```bash
mkdir -p frontend/src/features/upload frontend/src/features/results
echo "export function UploadPage() { return <div>Upload</div> }" > frontend/src/features/upload/UploadPage.tsx
echo "export function ResultsPage() { return <div>Results</div> }" > frontend/src/features/results/ResultsPage.tsx
```

Then write `frontend/src/App.tsx`:
...
- Sticky header with CassetteReader logo, nav pills (Upload active, Analysis disabled), theme toggle
- "Upload" placeholder text in the page body
- Theme toggle switches between light/dark, persists on refresh
...
```bash
git add frontend/src/App.tsx frontend/src/AppHeader.tsx \
        frontend/src/features/upload/UploadPage.tsx \
        frontend/src/features/results/ResultsPage.tsx
git commit -m "feat: app shell — routing, Layout, AppHeader with theme toggle"
```

---

### Task 10: Query hooks — `usePollJob`, `useResults`, `useRecentJobs`
...
Note: `CrosshairProvider` / `useCrosshair` are deferred to Task 20 where they're actually needed by `SignalsCard`.

- [ ] **Step 1: Create `frontend/src/hooks/usePollJob.ts`**
...
- [ ] **Step 2: Create `frontend/src/hooks/useResults.ts`**
...
- [ ] **Step 3: Create `frontend/src/hooks/useRecentJobs.ts`**
...
git commit -m "feat: usePollJob, useResults, useRecentJobs query hooks"
```

---

### Task 11: `Dropzone`, `OptionRow`, `UploadPage` (idle state)
...
- [ ] **Step 3: Rewrite `frontend/src/features/upload/UploadPage.tsx`** (idle state; polling added in Task 12)
...
- [ ] **Step 4: Verify idle state in browser**

Navigate to `http://localhost:3000`. You should see:
- Upload card with dropzone and two toggle rows
- Toggling "Include stems" works
- Analyze button disabled until a file is dropped
- Clicking Analyze with a file sets `?job=<uuid>` in the URL (page stays on idle — ProgressPanel wired in Task 12)
...
### Task 12: `ProgressPanel` + `RecentUploads` + full upload flow
...
- [ ] **Step 3: Rewrite `frontend/src/features/upload/UploadPage.tsx`** with polling + redirect wired in
...
3. Drop an audio file onto the dropzone → Analyze button activates
4. Click Analyze → URL becomes `/?job=<uuid>` → ProgressPanel appears with hatch bar + stage tiles
5. Tiles update as progress increases; on `status === done` → auto-redirects to `/results/<uuid>`
6. Navigate back to `/` — Recent analyses card shows the completed job with key/BPM/duration
...
```bash
git add frontend/src/features/upload/ProgressPanel.tsx \
        frontend/src/features/upload/RecentUploads.tsx \
        frontend/src/features/upload/UploadPage.tsx
git commit -m "feat: ProgressPanel, RecentUploads, full UploadPage with polling and redirect"
```

---

### Task 13: `HeroSummary` + `SubHeader` + basic `ResultsPage`
...
After this task `/results/:jobId` shows real data in the browser.
...
- [ ] **Step 4: Rewrite `frontend/src/features/results/ResultsPage.tsx`** with loading/error states and hero panel
...
Navigate to `/results/<any completed job_id>`. You should see:
- SubHeader with filename breadcrumb and JSON/Share buttons
- HeroSummary: large key display, animated BPM dot, progression fingerprint, duration + section/beat counts
...
git add frontend/src/styles/globals.css \
        ...
        frontend/src/features/results/ResultsPage.tsx
git commit -m "feat: HeroSummary, SubHeader, basic ResultsPage with loading/error states"
```

---

### Task 14: `SectionRibbon`, `Waveform`, `WaveformCard` (static — no audio yet)
...
`SectionRibbon` and `Waveform` accept `currentTime` and `onSeek` as props so they render correctly without `AudioProvider`. Task 19 wires them to real audio.
...
- [ ] **Step 3: Create `frontend/src/features/results/WaveformCard.tsx`**

`currentTime=0` and `onSeek=()=>{}` are placeholders — Task 19 replaces them with real audio values.
...
- [ ] **Step 4: Add `WaveformCard` to `ResultsPage`**

Replace the `{/* WaveformCard added in Task 14 */}` comment in `frontend/src/features/results/ResultsPage.tsx` with:

```tsx
import { useState } from 'react'
// ... other imports stay the same, add:
import { WaveformCard } from './WaveformCard'
```

Add `const [selectedChord, setSelectedChord] = useState<string | null>(null)` after the error check, and replace the comment with:
...
The full updated `ResultsPage.tsx`:
...
export function ResultsPage() {
  ...
  return (
    <main style={{ maxWidth: 1080, margin: '40px auto', padding: '0 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      <SubHeader result={result} />
      <HeroSummary result={result} />
      <WaveformCard result={result} selectedChord={selectedChord} onChordSelect={setSelectedChord} />
      {/* SignalsCard added in Task 15 */}
    </main>
  )
}
...
Navigate to `/results/<job_id>`. You should see:
- HeroSummary (key, BPM, progression, duration)
- Waveform card: coloured section bands, mirrored waveform peaks, beat ticks, chord ribbon at bottom
- Clicking a chord in the ribbon highlights it (selectedChord works)
- Hover over waveform shows time tooltip
- Playhead sits at 0:00 (audio not wired yet — that's Task 19)
...
git add frontend/src/features/results/SectionRibbon.tsx \
        ...
        frontend/src/features/results/ResultsPage.tsx
git commit -m "feat: SectionRibbon, Waveform SVG, WaveformCard (static — audio in Task 19)"
```

---

### Task 15: `ChordLane`, `BeatRuler`, `SignalLane`, `SignalsCard` (static — no hover crosshair yet)
...
All four SVGs share layout constants — copy these identically so lanes align when stacked:
...
Hover crosshair and `CrosshairProvider` are added in Task 20. These components just render data.
...
No `CrosshairProvider` yet — hover sync is added in Task 20.
...
- [ ] **Step 5: Add `SignalsCard` to `ResultsPage`**

Replace the `{/* SignalsCard added in Task 15 */}` comment in `ResultsPage.tsx`. Add the import and the component:
...
Navigate to `/results/<job_id>`. Below the waveform you should see:
- Chord lane with clickable chord blocks (clicking also highlights in waveform ribbon — selectedChord shared)
- Tension polyline (accent colour)
- Loudness polyline (teal)
- Brightness polyline (purple)
- Density step-function (green)
- Beat ruler with downbeat/beat ticks at the bottom
...
git add frontend/src/features/results/ChordLane.tsx \
        ...
        frontend/src/features/results/ResultsPage.tsx
git commit -m "feat: ChordLane, BeatRuler, SignalLane, SignalsCard (static — crosshair in Task 20)"
```

---

### Task 16: `PitchClassCard`
...
- [ ] **Step 2: Add `PitchClassCard` to `ResultsPage`**

Add import and render it in a 2-column grid below SignalsCard. Update `ResultsPage.tsx`:

```tsx
import { PitchClassCard } from './PitchClassCard'
```

Replace the `{/* PitchClassCard + InstrumentsCard added in Tasks 16–17 */}` comment with:
...
Navigate to `/results/<job_id>`. Below the signal lanes you should see:
- Left column: 12-segment radial SVG wheel — tonic in accent colour, avoid notes darker
- Hover over a slice → note name + percentage appears in the centre
- Right column: top-4 bars (InstrumentsCard placeholder slot visible)
...
### Task 17: `Knob`, `InstrumentsCard`, `SectionComparison`, `TranspositionCard`
...
5 positions (−4/−2/0/+2/+4 semitones). Clicking anywhere on the face snaps to the nearest position based on pointer angle.
...
- [ ] **Step 2: Create `frontend/src/features/results/InstrumentsCard.tsx`**
...
- [ ] **Step 3: Create `frontend/src/features/results/SectionComparison.tsx`**
...
- [ ] **Step 4: Create `frontend/src/features/results/TranspositionCard.tsx`**
...
- [ ] **Step 5: Add remaining cards to `ResultsPage`**

Full updated `frontend/src/features/results/ResultsPage.tsx`:
...
- [ ] **Step 6: Verify in browser — all data visible**

Navigate to `/results/<job_id>`. The full results page should now show all cards:
- HeroSummary, WaveformCard, SignalsCard
- 2-column grid: PitchClassCard (left) + InstrumentsCard (right, if stems present)
- SectionComparison table with mini bar per cell
- TranspositionCard with Knob (only if transposition data present): clicking knob snaps to position, row highlights
...
git add frontend/src/components/Knob.tsx \
        frontend/src/features/results/InstrumentsCard.tsx \
        frontend/src/features/results/SectionComparison.tsx \
        frontend/src/features/results/TranspositionCard.tsx \
        frontend/src/features/results/ResultsPage.tsx
git commit -m "feat: Knob, InstrumentsCard, SectionComparison, TranspositionCard — all result data visible"
```

---

### Task 18: `AudioProvider` + `Transport` — add audio playback

**Files:**
- Create: `frontend/src/hooks/useAudio.tsx`
- Create: `frontend/src/features/results/Transport.tsx`
- Modify: `frontend/src/features/results/SectionRibbon.tsx`
- Modify: `frontend/src/features/results/Waveform.tsx`
- Modify: `frontend/src/features/results/WaveformCard.tsx`
- Modify: `frontend/src/features/results/ResultsPage.tsx`

After this task the waveform has a moving playhead and Transport controls play/pause/skip sections.
...
- [ ] **Step 2: Create `frontend/src/features/results/Transport.tsx`**
...
export function Transport({ sections, duration }: TransportProps) {
  ...
  return (
    <div style={{
      ...
      <button onClick={skipPrev} className="btn ghost icon-only"
        title="Previous section" style={{ color: 'var(--ink-2)' }}>
      ...
      <button onClick={skipNext} className="btn ghost icon-only"
        title="Next section" style={{ color: 'var(--ink-2)' }}>
      ...
      <span className="d-italic-explain" style={{ fontSize: 11 }}>
        click the waveform to seek · prev/next jumps sections
      </span>
    </div>
  )
}
...
- [ ] **Step 3: Update `SectionRibbon` to use `useAudio()`**

Replace the `currentTime` / `onSeek` props with the hook. Rewrite `frontend/src/features/results/SectionRibbon.tsx`:
...
- [ ] **Step 4: Update `Waveform` to use `useAudio()`**

Replace the `currentTime` / `onSeek` props with the hook. Rewrite `frontend/src/features/results/Waveform.tsx` — same code as Task 14 but replace the props with:

```tsx
import { useAudio } from '../../hooks/useAudio'
// remove currentTime and onSeek from WaveformProps
// add at top of component body:
const { currentTime, seek } = useAudio()
// replace onSeek with seek in onClick handler
```

Full rewrite of `frontend/src/features/results/Waveform.tsx`:
  ...
  return (
    ...
    >
      ...
      {peaks.map((p, i) => {
        ...
        const barW = W / peaks.length
        return (
          <rect key={i} x={i * barW} y={midY - barH}
            width={Math.max(barW - 0.5, 0.5)} height={barH * 2}
            fill="var(--ink-3)" opacity={0.55}
          />
        )
      ...
      {beats.map((t, i) => {
        ...
        const isDown = downbeatsSet.has(String(t))
        return (
          <line key={i} x1={x} y1={BEAT_Y_START + (isDown ? 0 : 8)} x2={x} y2={CHORD_Y_START}
            stroke={isDown ? 'var(--ink-2)' : 'var(--ink-4)'}
            strokeWidth={isDown ? 1.5 : 0.75}
          />
        )
      ...
      {chords.map((c, i) => {
        ...
        return (
          <g key={i} style={{ cursor: 'pointer' }}
            onClick={(e) => { e.stopPropagation(); onChordSelect(isSelected ? null : c.roman) }}
          >
            <rect x={x1} y={CHORD_Y_START} width={x2 - x1 - 0.5} height={H - CHORD_Y_START}
              fill={isSelected ? 'var(--accent-soft)' : 'var(--paper-2)'}
              stroke={isSelected ? 'var(--accent)' : 'var(--rule)'}
              strokeWidth={isSelected ? 1 : 0.5}
            />
            {x2 - x1 > 18 && (
              <text x={(x1 + x2) / 2} y={H - 4} textAnchor="middle" fontSize={8}
                fontFamily="var(--font-mono)"
                fill={isSelected ? 'var(--accent-ink)' : 'var(--ink-3)'}
                style={{ pointerEvents: 'none' }}
              >
      ...
      <line x1={playheadX} y1={0} x2={playheadX} y2={H}
        stroke="var(--accent)" strokeWidth={1.5} pointerEvents="none"
      />
      <circle cx={playheadX} cy={4} r={4} fill="var(--accent)" pointerEvents="none"
      />
    </svg>
  )
}
...
- [ ] **Step 5: Update `WaveformCard` — wrap in `AudioProvider`, add `Transport`**

Rewrite `frontend/src/features/results/WaveformCard.tsx`:
...
- [ ] **Step 6: Remove `AudioProvider` import from `ResultsPage`** (it now lives in `WaveformCard`)

`ResultsPage.tsx` requires no changes — `WaveformCard` self-contains the provider.

- [ ] **Step 7: Verify audio playback in browser**

Navigate to `/results/<job_id>`:
1. Click Play button → audio plays, playhead moves across waveform
2. Click anywhere on waveform → seeks to that position
3. Click a section button in the ribbon → seeks to that section start
4. Prev/Next buttons skip between section boundaries
5. Screen shows `currentTime / duration` updating in real time
...
git add frontend/src/hooks/useAudio.tsx \
        ...
        frontend/src/features/results/SectionRibbon.tsx \
        frontend/src/features/results/Waveform.tsx \
        frontend/src/features/results/WaveformCard.tsx
git commit -m "feat: AudioProvider, Transport, audio playback wired into WaveformCard"
```

---

### Task 19: CrosshairProvider + hover sync across signal lanes

**Files:**
- Create: `frontend/src/context/CrosshairProvider.tsx`
- Modify: `frontend/src/features/results/ChordLane.tsx`
- Modify: `frontend/src/features/results/BeatRuler.tsx`
- Modify: `frontend/src/features/results/SignalLane.tsx`
- Modify: `frontend/src/features/results/SignalsCard.tsx`

- [ ] **Step 1: Write the failing test**

`frontend/src/context/CrosshairProvider.test.tsx`:
...
```bash
cd frontend && npx vitest run src/context/CrosshairProvider.test.tsx
```

Expected: FAIL — module not found.

- [ ] **Step 3: Write `CrosshairProvider.tsx`**

`frontend/src/context/CrosshairProvider.tsx`:
...
- [ ] **Step 5: Update `ChordLane.tsx` to use `useCrosshair`**

The lane already renders chord blocks. Add a vertical crosshair line and hover tracking.

Full replacement for `frontend/src/features/results/ChordLane.tsx`:

```tsx
import { useMemo } from 'react'
import { ChordEntry } from '../../types/api'
import { sectionColor } from '../../lib/utils'
import { useCrosshair } from '../../context/CrosshairProvider'

const W = 1000, H = 48, LABEL_W = 72, RIGHT_W = 36
...
export function ChordLane({ chords, duration }: Props) {
  const { hoverTime, setHoverTime } = useCrosshair()
  const trackW = W - LABEL_W - RIGHT_W

  const blocks = useMemo(() => chords.map((c) => ({
    x: LABEL_W + (c.time / duration) * trackW,
    w: (c.duration / duration) * trackW,
    label: c.chord,
    color: sectionColor(c.chord),
  })), [chords, duration, trackW])

  const crossX = hoverTime != null ? LABEL_W + (hoverTime / duration) * trackW : null

  function handleMouseMove(e: React.MouseEvent<SVGSVGElement>) {
    const rect = e.currentTarget.getBoundingClientRect()
    const px = e.clientX - rect.left
    const ratio = (px - LABEL_W * (rect.width / W)) / (trackW * (rect.width / W))
    setHoverTime(Math.max(0, Math.min(1, ratio)) * duration)
  }

  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}
      onMouseMove={handleMouseMove} onMouseLeave={() => setHoverTime(null)}>
      <text x={LABEL_W - 8} y={H / 2 + 5} textAnchor="end"
        fontSize={11} fill="var(--color-text-muted)">Chords</text>
      {blocks.map((b, i) => (
        <g key={i}>
          <rect x={b.x} y={4} width={b.w} height={H - 8} fill={b.color} opacity={0.7} rx={2} />
          {b.w > 30 && (
            <text x={b.x + b.w / 2} y={H / 2 + 4} textAnchor="middle"
              fontSize={10} fill="var(--color-text-primary)" pointerEvents="none">{b.label}</text>
          )}
        </g>
      ))}
      {crossX != null && (
        <line x1={crossX} y1={0} x2={crossX} y2={H}
          stroke="var(--color-accent)" strokeWidth={1} pointerEvents="none" />
      )}
    </svg>
  )
}
...
- [ ] **Step 6: Update `BeatRuler.tsx` to use `useCrosshair`**

Full replacement for `frontend/src/features/results/BeatRuler.tsx`:

```tsx
import { useMemo } from 'react'
import { BeatInfo } from '../../types/api'
import { formatTime } from '../../lib/utils'
import { useCrosshair } from '../../context/CrosshairProvider'

const W = 1000, H = 28, LABEL_W = 72, RIGHT_W = 36
...
export function BeatRuler({ beats, duration }: Props) {
  const { hoverTime, setHoverTime } = useCrosshair()
  const trackW = W - LABEL_W - RIGHT_W

  const ticks = useMemo(() => beats.map((b) => ({
    x: LABEL_W + (b.time / duration) * trackW,
    isMeasure: b.beat_in_measure === 1,
  })), [beats, duration, trackW])
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}
      onMouseMove={handleMouseMove} onMouseLeave={() => setHoverTime(null)}>
      {ticks.map((t, i) => (
        <line key={i} x1={t.x} y1={t.isMeasure ? 0 : H / 2} x2={t.x} y2={H}
          stroke={t.isMeasure ? 'var(--color-text-muted)' : 'var(--color-border)'}
          strokeWidth={t.isMeasure ? 1 : 0.5} />
      ))}
      {crossX != null && (
        <>
          <line x1={crossX} y1={0} x2={crossX} y2={H}
            stroke="var(--color-accent)" strokeWidth={1} pointerEvents="none" />
          {hoverLabel && (
            <text x={Math.min(crossX + 4, W - 40)} y={H - 4}
              fontSize={9} fill="var(--color-accent)">{hoverLabel}</text>
          )}
        </>
      )}
    </svg>
...
- [ ] **Step 7: Update `SignalLane.tsx` to use `useCrosshair`**

Full replacement for `frontend/src/features/results/SignalLane.tsx`:
...
const W = 1000, H = 60, LABEL_W = 72, RIGHT_W = 36
...
export function SignalLane({ label, series, color = 'var(--color-accent)', fillOpacity = 0.15 }: Props) {
  const { hoverTime, setHoverTime } = useCrosshair()
  const trackW = W - LABEL_W - RIGHT_W

  const { points, hoverIdx, hoverVal } = useMemo(() => {
    const pts = series.times.map((t, i) => ({
      x: LABEL_W + (t / (series.times[series.times.length - 1] || 1)) * trackW,
      y: H - 4 - series.values[i] * (H - 8),
    }))
    let hoverIdx: number | null = null
    if (hoverTime != null) {
      let best = 0
      for (let i = 1; i < series.times.length; i++) {
        if (Math.abs(series.times[i] - hoverTime) < Math.abs(series.times[best] - hoverTime)) best = i
      }
      hoverIdx = best
    }
    return {
      points: pts,
      hoverIdx,
      hoverVal: hoverIdx != null ? series.values[hoverIdx] : null,
    }
  }, [series, trackW, hoverTime])

  const crossX = hoverTime != null
    ? LABEL_W + (hoverTime / (series.times[series.times.length - 1] || 1)) * trackW
    : null

  const polyline = points.map((p) => `${p.x},${p.y}`).join(' ')
  const area = points.length > 0
    ? `M${points[0].x},${H - 4} ` + points.map((p) => `L${p.x},${p.y}`).join(' ') + ` L${points[points.length - 1].x},${H - 4} Z`
    : ''

  function handleMouseMove(e: React.MouseEvent<SVGSVGElement>) {
    const rect = e.currentTarget.getBoundingClientRect()
    const px = e.clientX - rect.left
    const duration = series.times[series.times.length - 1] || 1
    const ratio = (px - LABEL_W * (rect.width / W)) / (trackW * (rect.width / W))
    setHoverTime(Math.max(0, Math.min(1, ratio)) * duration)
  }
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}
      onMouseMove={handleMouseMove} onMouseLeave={() => setHoverTime(null)}>
      <text x={LABEL_W - 8} y={H / 2 + 4} textAnchor="end"
        fontSize={11} fill="var(--color-text-muted)">{label}</text>
      {area && <path d={area} fill={color} opacity={fillOpacity} />}
      {polyline && <polyline points={polyline} fill="none" stroke={color} strokeWidth={1.5} />}
      {crossX != null && (
        <line x1={crossX} y1={0} x2={crossX} y2={H}
          stroke="var(--color-accent)" strokeWidth={1} pointerEvents="none" />
      )}
      {hoverIdx != null && points[hoverIdx] && (
        <>
          <circle cx={points[hoverIdx].x} cy={points[hoverIdx].y} r={3}
            fill={color} stroke="var(--color-bg-primary)" strokeWidth={1.5} />
          <text x={Math.min(points[hoverIdx].x + 5, W - 40)} y={points[hoverIdx].y - 4}
            fontSize={9} fill={color}>{hoverVal?.toFixed(2)}</text>
        </>
      )}
    </svg>
  )
...
- [ ] **Step 8: Wrap `SignalsCard.tsx` in `CrosshairProvider`**

Full replacement for `frontend/src/features/results/SignalsCard.tsx`:

```tsx
import { AnalysisResult } from '../../types/api'
import { downsampleRms } from '../../lib/utils'
import { Card } from '../../components/Card'
...
export function SignalsCard({ result }: Props) {
  const rms = result.dynamics?.rms
  const tension = result.tension?.tension_curve
  const downsampled = rms ? downsampleRms(rms, 720) : null

  return (
    <CrosshairProvider>
      <Card>
        <h3 style={{ margin: '0 0 12px', fontSize: 14, color: 'var(--color-text-muted)' }}>
          Signals
        </h3>
        <ChordLane chords={result.chords ?? []} duration={result.duration_sec} />
        <BeatRuler beats={result.beats ?? []} duration={result.duration_sec} />
        {downsampled && (
          <SignalLane label="RMS" series={downsampled}
            color="var(--color-accent)" fillOpacity={0.15} />
        )}
        {tension && (
          <SignalLane label="Tension" series={tension}
            color="var(--color-warning, #f59e0b)" fillOpacity={0.12} />
        )}
      </Card>
    </CrosshairProvider>
  )
...
Expected: all tests pass including the new CrosshairProvider tests.
...
- [ ] **Step 11: Verify hover sync in browser**

Navigate to `/results/<job_id>`. In the SignalsCard:
1. Move mouse over ChordLane → vertical crosshair appears across all three lanes simultaneously
2. Move mouse over BeatRuler → time label updates, all lanes track
3. Move mouse over RMS/Tension signal → value dot appears on line, crosshair visible in all lanes
4. Mouse leaves any lane → crosshair disappears from all lanes

- [ ] **Step 12: Commit**

```bash
git add frontend/src/context/CrosshairProvider.tsx \
        frontend/src/context/CrosshairProvider.test.tsx \
        frontend/src/features/results/ChordLane.tsx \
        frontend/src/features/results/BeatRuler.tsx \
        frontend/src/features/results/SignalLane.tsx \
        frontend/src/features/results/SignalsCard.tsx
git commit -m "feat: CrosshairProvider — synchronized hover time across signal lanes"
```

---

### Task 20: FastAPI StaticFiles mount + final end-to-end verification

**Files:**
- Modify: `app/main.py`

- [ ] **Step 1: Add conditional StaticFiles mount to `app/main.py`**

Read the current `app/main.py` and add at the bottom, after all routers are registered:
...
The `html=True` flag makes FastAPI serve `index.html` for any unmatched path, enabling React Router's client-side routing.

**Why conditional:** `frontend/dist/` only exists after `npm run build`. In dev mode the Vite dev server is used instead. The backend always starts cleanly whether or not the dist exists.

- [ ] **Step 2: Verify dev mode still works**

Without a `frontend/dist/` directory:

```bash
uvicorn app.main:app --reload --port 8000
```

Expected: server starts, no error about missing directory.

- [ ] **Step 3: Build frontend and verify production mode**

```bash
cd frontend && npm run build
```

Expected output ends with: `dist/index.html` and `dist/assets/index-*.js` listed.

```bash
cd .. && uvicorn app.main:app --port 8000
```

Navigate to `http://localhost:8000/` — the React app should load (served by FastAPI).
Navigate to `http://localhost:8000/results/anything` — React app loads (not a 404), client-side routing handles it.

- [ ] **Step 4: Full end-to-end test in production mode**

With `uvicorn app.main:app --port 8000` running:

1. Open `http://localhost:8000/`
2. Upload a real audio file (MP3 or WAV)
3. Watch the progress panel — all 9 stages should tick through
4. On completion, the browser redirects to `/results/<job_id>`
5. HeroSummary shows: key name, tempo, chord progression, duration
6. WaveformCard shows: section ribbon, waveform, Transport controls
7. Click Play → audio plays
8. SignalsCard shows: ChordLane, BeatRuler, RMS lane (Tension if stage 7 implemented)
9. Hover across SignalsCard → crosshair syncs across all lanes
10. PitchClassCard shows the 12-segment donut with tonic highlighted
11. InstrumentsCard, SectionComparison, TranspositionCard render without errors
12. Dark/light theme toggle in AppHeader works
13. Open DevTools → no console errors

- [ ] **Step 5: Commit**

```bash
git add app/main.py
git commit -m "feat: serve Vite dist via FastAPI StaticFiles in production"
```

- [ ] **Step 6: Commit the completed v2 plan**

```bash
git add docs/superpowers/plans/2026-05-21-frontend-v2.md
git commit -m "docs: complete frontend v2 implementation plan (Tasks 1-20)"
```

---

*Plan complete. 20 tasks, ~8 hours of focused implementation. Tasks 1–3 are backend-only; Tasks 4–20 are frontend. Each task ends with a browser verification step so regressions are caught immediately.*
